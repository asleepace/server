...
export function bootstrapSessionPage() {
  ...
  // Populate shared store for binding
  App.store.set('linkSnippet', `${location.origin}/s/${sessionKey}`)
  App.store.set('curlSnippet', curl)
  App.store.set('jsSnippet', js)
  App.store.set('pySnippet', py)
  App.store.set('activeTab', 'instructions')
  ...
  document.addEventListener('click', (e) => {
    ...
    if (act.startsWith('dispatch:')) {
      ...
      btn.dispatchEvent(new CustomEvent(name, { bubbles: true, composed: true }))
